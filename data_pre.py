...
def load_credit():
    df = pd.read_csv(r'/home/SSD/hiran/imbDRL_Data/creditcard.csv', low_memory=False)
    x = df.iloc[:,:-1]
    y = df['Class']
    scaler = StandardScaler()
    x = scaler.fit_transform(x)
    x_train, x_test, y_train, y_test = train_test_split(x, y, test_size=0.15)
    #print("X_train, y_train:",X,y, X_train.shape,y_train.shape)
    #print("X_test,y_test:",X,y, X_test.shape,y_test.shape)
    return x_train, y_train, x_test, y_test
